...
Best card is the **Pay** view of [Rewards & benefits](REWARDS.md) now: the
comparison is the purchase advisor's, and the card terms it rests on are under
**Card rates** on that view. `cards.html` and the `cards` capability id stay as
ways in and open Rewards on Pay; there is no second add-card form and no second
calculator. What follows describes the terms and the comparison as they work
there. To add a card, say which card you have however you name it — `chase sapphire`, `amex gold`, `my citi 2% card` — and select Find card and rewards. Research using a saved OpenAI connection identifies the exact product from the issuer's current pages and fills in the whole card: reward type, base rate, what a point is worth, every bonus category, enrollment, caps, exclusions, the issuer link and today's review date. A name that fits more than one real card returns the products it could be, each with what separates it, and researches nothing until one is chosen. The summary lists every rate that was ingested; Card terms holds the same values as ordinary fields and stays shut, because nothing is left for you to fill in. A point's value is the one the program itself publishes for this card — its cash or statement-credit value, or its travel-portal value with any boost this card gets, whichever is higher — never a transfer-partner estimate, and the notes say which it is. If none can be found, 1¢ is assumed and the notes say so. Saving is always yours: review the summary or the terms, then save. Without a connection or internet, Card terms accepts a card entered by hand. No card account numbers, security codes or bank credentials are collected.
...
The application computes rewards from saved rates rather than asking AI to choose the winner: AI reads the purchase, and saved terms decide the card. Each result names the reward program that applied — the matching bonus, or the base rate when no bonus matched — and how far it is from the next card in points and dollars. Cash back and points are compared as estimated dollars using each card's cents-per-point value, which is the owner's planning value and not a fact about the purchase: Pay says where it stops mattering — the value a point would have to carry for the points card to beat the best cash return — and the Points view keeps a value per currency, with its source, that outranks the card's. It displays ties, bonus caps and base-rate fallback. It does not combine overlapping bonuses. Expired, inactive, wrong-channel and unconfirmed bonuses are excluded. Conflicted cards and pending deletions are excluded until resolved.

## Limits

The issuer determines the actual merchant category. Estimates exclude fees, interest, signup bonuses, and unentered offers. Rewards may depend on card variants, merchant exclusions, account tiers, payment methods and purchase country. Review these against the linked terms. A point's value is the program's own published redemption value, not a guaranteed rate; change it in Card terms if you redeem differently.
