...
A card the issuer publishes nothing about — invitation-only, private bank, or offered only as an upgrade — is researched from the product it is a version of, where there is one: the name stays the card you hold, the source is the published product's page, and the notes say which product the rates came from. Borrowed rates are never presented as the card's own confirmed ones. Reading the card's own page is the better source, and writes the rates it states to that card's terms.
