...
A group's label is never set below the records it heads, and a record's own actions never form a row of words beneath it: both rules are in [DESIGN.md](DESIGN.md#row-actions), and `GroupTitle`, `RecordGroup`, `RecordRow`, `RowAction` and `RowLink` implement them. `RecordGroup` is that label with its run of records under it; which run a record belongs to is decided in the feature's data module, never in the controller that draws it. `RecordRow` puts its `actions` on the record's name line and its `extra` — a confirmation, a conflict, a revealed value — inside the row under that line.
