...
Use the shared app header and Tools navigation, followed by the selected tool's heading, content, and actions. Tools come from the shared capabilities registry. Current tab is the automatic mode the sidebar starts in; the Tools toggle names it and the menu has no row for it. A manual selection stays selected when browser tabs change, marked in the menu by its highlighted row alone. Under the header, and above whichever tool is open, sits the page strip: what these tools can do with the page in front of the owner, as one row of destinations. It is not a second tool list and must not become one — every entry is a capability the Tools menu already leads to, it appears only when the page offers something, it never offers the tool already on screen, and it changes nothing until it is pressed. Recognizing a page is not a reason to move the owner: Automatic mode still follows the tab, and a tool chosen by hand stays chosen. Do not add new function tabs, feature shortcuts, or a second navigation system to the header.
