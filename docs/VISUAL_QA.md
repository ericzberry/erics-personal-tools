...
| Feature fixtures | The same server exposes `/tests/travel-editor-preview.html` (the travel wallet against synthetic records: three categories, a name long enough to wrap, one record with notes and one with an expiry, plus the editor the sidebar opens in its own tab), `/tests/restaurant-preview.html`, `/tests/draft-preview.html`, `/tests/vault-gate-preview.html` (the passkey gate's waiting, dismissed and open states with a synthetic passkey; the open state repeats for an ordinary page and for each account site the sidebar recognizes, so Finance's page reading can be reviewed either way, and two quiet-arrival states show the panel as it opens beside a finance page — figures unbuilt, intake ready — with and without a site it can read), `/tests/page-strip-preview.html` (the page strip's states, one per kind of page it recognizes), `/tests/balances-preview.html` (the wallet's points and miles: its per-unit totals, and the balance panel beside a program's page before a reading, after one, and when the page shows no balance), `/tests/status-tones-preview.html` (the four status tones side by side, plus the spinner and the progress bar, where a change to what a status looks like is reviewed), `/tests/subscriptions-preview.html` (the statement intake with a saved connection and without one, a wallet of saved services, and what a dropped file leaves behind: read, half read, and not read at all), `/tests/ai-models-preview.html` (the AI models list in Settings: one row per action, an automatic default named and unnamed, and choices already made), `/tests/finance-ledger-preview.html` (the side panel's Finance **Net worth** block against synthetic figures — the figure, how much is liquid, each entity's total and **Open details**: a ledger still being filled in, where one reading covers every figure and the one before it covers half of them, a settled ledger with liabilities, a private position, two properties and a stale portfolio (so the line saying how much of the figure is more than 90 days old shows), a house saved with its Zillow page and no figure yet, and a ledger with nothing in it), `/tests/finance-page-preview.html` (the ledger's own page, which is also the phone's Finance tool, loading the sheets `finance.html` loads: `?state=year` for five full quarters, a drawn line, and cash moved at three institutions — one movement before a firm's first full reading and one after its latest — so every state an institution card's cash can be in is shown, `today` for one quarter and no line, `aging` for that quarter with a trust, a fund's statement and a house left more than 90 days behind, so the stale amounts and **Sources** have something to account for, `sparse` for one entity with nothing private and no houses, and `empty`; ages are measured against a fixed day; review it at 1440px, around 880px where the tables stack, and 390px), `/tests/home-preview.html` (the home screen's weather and its three runs: one birthday today with three behind it and three credits before the quarter closes under a rainy light-jacket day, two today and nothing after under a partly cloudy sweater day with no place named, nobody today with money about to reset under a windy snow day with a long place name, more credits than a glance holds under a warm sunny day, the weather alone on a foggy day and on a day with storms twice, nothing in the fortnight and nothing in the wallet with no forecast to be had, and not connected), `/tests/storage-preview.html` (the Settings screen's cloud storage panel: an account using almost nothing, one at each threshold, the account limit rather than the database one, a reading that could not be taken again, and a Worker never given the Cloudflare token). Inspect each harness before use and confirm it exercises the changed path; a feature fixture alone does not verify navigation into it. |
...
A harness links the sheets its host links, in the same order. Three of the side panel's stylesheets import `travel.css` again, so the shared rules are re-inserted after every sheet that means to override them; a preview that stops at the feature's own sheet shows a cascade the owner never sees, and an override that was only winning on source order looks correct in it. See UI-31 and UI-32 in [UI_RULES.md](UI_RULES.md).
