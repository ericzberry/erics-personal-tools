...
test('the intake model labels what it reads and is never asked to total, match or classify blind',async()=>{
  ...
  const fetcher=async(url,options)=>url.endsWith('/models')
    ?Response.json({data:[{id:'gpt-5.6-luna'},{id:'gpt-5.6-terra'},{id:'gpt-5-mini'}]})
...
test('an image is read under the same rules as text and never sent to a model that cannot see',async()=>{
  ...
  const result=await readFinanceUpdates(connection,{images:[PIXEL],today:'2026-07-01'},models(['gpt-5.6-luna','gpt-5.6-terra','gpt-5-mini']));
  ...
  await assert.rejects(readFinanceUpdates(connection,{},models(['gpt-5.6-luna','gpt-5.6-terra','gpt-5-mini'])),error=>error.status===400);
  await assert.rejects(readFinanceUpdates(connection,{images:Array(MAX_INTAKE_IMAGES+1).fill(PIXEL)},models(['gpt-5.6-luna','gpt-5.6-terra','gpt-5-mini'])),error=>error.status===400);
  // A malformed data URL never reaches the provider.
  await assert.rejects(readFinanceUpdates(connection,{images:['https://example.com/x.png']},models(['gpt-5.6-luna','gpt-5.6-terra','gpt-5-mini'])),error=>error.status===400);
});
