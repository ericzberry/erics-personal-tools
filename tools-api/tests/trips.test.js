...
test('trip migration preserves old data; writes are encrypted, authenticated and revision guarded',async()=>{
  const sql=new DatabaseSync(':memory:');
  sql.exec('CREATE TABLE old_records (value TEXT); INSERT INTO old_records VALUES (\'preserve me\');');
  const schema=readFileSync(new URL('../trips-schema.sql',import.meta.url),'utf8');sql.exec(schema);sql.exec(schema);
  ...
  const path='/v1/trips/22222222-2222-4222-8222-222222222222';
  ...
  const value={schemaVersion:1,title:'Private test trip',kind:'hotel',request:'Two bedrooms near a private destination.'};
  ...
  assert.equal(sql.prepare('SELECT value FROM trip_records').get().value.includes('Private test'),false);
  const snapshot=await call('/v1/trips/snapshot');assert.match(snapshot.headers.get('cache-control'),/no-store/);
  assert.equal((await snapshot.json()).records[0].request,value.request);
  assert.equal((await call(path,'PUT',value)).status,409);
  assert.equal((await call(path,'DELETE',{revision:saved.revision})).status,200);
  assert.equal((await (await call('/v1/trips')).json()).records.length,0);
});
