...
test('the finance reading takes the fast model, and an image still finds one that can see',()=>{
  ...
  // Not the smallest model that would answer. The one below this could not
  // carry a page's group headings down onto the accounts under them, so a
  // family's four entities came apart into twenty-eight portfolios named after
  // account numbers. What this task buys at this tier is rule-following.
  const small=MODEL_CATALOG.find(model=>model.id==='gpt-5.6-luna');
  if(small)assert.notEqual(chosen.model.id,small.id,'the small model lost the holder headings');
