...
export async function researchCard(connection,input,fetcher=fetch){
  ...
  try{
    ...
    if(!outputs.some(o=>o.type==='web_search_call'))throw Error('No supporting issuer source was returned. Add the card manually or retry.');
    ...
    if(!source||!sources.some(url=>issuerSourceKey(url)===source))throw Error('No supporting issuer source was returned. Add the card manually or retry.');
