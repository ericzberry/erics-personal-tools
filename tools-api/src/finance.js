...
${live?`This text was read from an account page the owner is signed in to right now. Report one update per account the page names — its own account name and its own total or net value — never an individual holding inside an account, and never a figure summed across accounts. Leave out anything that is not one of the owner's accounts: a portfolio-wide or grand total across all of them, a day or period change, a market or index quote, a rate or price the page advertises, and any figure in its news, education or promotional panels. A balance the page shows without a date of its own is current, so give it asOf ${today} instead of dropping it; a figure the page itself dates keeps that date.${institution?` The institution is ${institution} unless the text names a different one.`:''}
