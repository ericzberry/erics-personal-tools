...
unread: one or two sentences naming a balance, position or capital account the owner would expect to find in "readings" or "capital" and will not — because it is illegible, undated, or cannot be tied to an account — and why. The owner reads it as a problem to act on, so it is "" whenever nothing is missing. Never name a figure these instructions tell you to leave out, or one that is a part of something you did report — a gain or loss, income, fees, an opening balance, a period's change, an amount over or under paid, a fund's own totals: leaving those out is the reading working, not a gap in it.
...
`:''}Return only figures the source actually states. An amount the source abbreviates to a scale is that amount written out — 341K is 341000, 1.2M is 1200000, 1.4B is 1400000000 — and writing one out is not computing it. Do not invent accounts, estimate values, annualize, or fill in a missing number.${images.length?' When a figure is blurred, cropped or otherwise not legible, leave it out and name it under unread rather than guessing at the digits.':''}`},
    {role:'user',content:[
