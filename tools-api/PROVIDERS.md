...
Named integrations use their official host, with optional same-host path overrides (such as Z.AI's coding path). For another host, select Custom and save its credential. IP literals, local hostnames, query strings and embedded credentials are rejected. Changing provider, endpoint or custom format clears the old key unless replaced. Redirects and automatic retries are disabled. Response bodies are limited to 2 MB. A call names its own deadline: a task's is derived from its output budget (15 ms per token, floored at 25 seconds and capped at 120), a research call allows 120 seconds, and anything else keeps 25. A timed-out request may still be billed. Provider errors are summarized without reflecting raw error bodies or keys.
