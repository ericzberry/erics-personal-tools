...
test('measurements lead, brands follow, and each row says the size itself',async()=>{
  ...
  assert.deepEqual([...document.querySelectorAll('#sizes-list .record-group-title')].map(node=>node.textContent),['General','Lululemon']);
  const rows=[...document.querySelectorAll('#sizes-list .record-row')];
  assert.equal(rows[0].querySelector('strong').textContent,'Waist · 33 in');
  assert.equal(rows[1].querySelector('strong').textContent,'ABC joggers · M');
  // How it runs is the thing that makes a bare size usable months later.
  assert.match(rows[1].textContent,/Runs slim/);
  ...
  const row=[...document.querySelectorAll('#sizes-list .record-row')][0];
