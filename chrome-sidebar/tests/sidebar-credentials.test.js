...
test('sidebar saves keys through the Worker, reports failures, and deletes using cloud revisions',async()=>{
 ...
 mountApp(document.getElementById('app'));let records=[],fail=false;const calls=[];
 globalThis.chrome={runtime:{sendMessage:async message=>{
  calls.push(message);
  if(message.action==='status')return {ok:true,connected:true};
  if(message.action==='list')return {ok:true,connections:records};
 ...
 assert.equal(calls.find(m=>m.action==='save').connection.apiKey,'synthetic-key');assert.equal($('credential-secret').value,'');assert.equal($('credential-status').textContent,'Saved in D1.');
