...
test('background pipeline saves picks and marks closed tab disconnected',async()=>{
  ...
  globalThis.chrome={omnibox:{setDefaultSuggestion:()=>{},onInputEntered:{addListener(){}}},contextMenus:{onClicked:{addListener(){}}},sidePanel:{setPanelBehavior:async()=>{}},runtime:{onInstalled:{addListener(){}},onStartup:{addListener(){}},onMessage:{addListener(fn){listener=fn;}}},storage:{local:{get:async()=>structuredClone(data),set:async v=>{data=structuredClone(v);},remove:async()=>{}},onChanged:{addListener(){},removeListener(){}}},tabs:{onRemoved:{addListener(fn){closed=fn;}},onUpdated:{addListener(){}},onActivated:{addListener(){}}}};
...
test('build excludes personal captures, tests and dependencies',()=>{
  ...
  const manifest=JSON.parse(readFileSync(new URL('../dist/manifest.json',import.meta.url)));// Geolocation is the home screen's weather: without it a side panel cannot ask.
  assert.deepEqual(manifest.permissions,['sidePanel','contextMenus','storage','scripting','geolocation']);assert.deepEqual(manifest.host_permissions,['<all_urls>']);
});
