...
function show({open=false,drafting=false,drafted=false,learned=false,connected=true,intent=''}){
  ...
  $('email-action-status').textContent=drafting?'Drafting in your voice…':drafted?'Review and edit before using.':'';
  $('email-result').hidden=!drafted;
  $('email-result-title').textContent='Reply draft';
  $('email-output').value=drafted?reply:'';
  $('email-output').dispatchEvent(new Event('output-updated'));
  voiceState={profile:learned?profile:null,scan:null,google:{connected,sentMail:connected}};
  page=learned?4:0;
  $('email-voice').open=learned||!connected;
  voice.load({force:true});
}
const states=[
