...
## One row of connection actions — 0.6.59

Settings → Credentials shows only the actions that apply: the access token with Connect while disconnected, and Refresh and Disconnect on a single row once connected. AI connections keeps Add connection and Reload connections together on one row and hides them until the browser is connected.
