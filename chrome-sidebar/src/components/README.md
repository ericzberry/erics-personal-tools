...
`IconButton` is what a record's own actions look like once the record is one line. It takes a 24x24 stroked path and the label that names the record it would act on — "Edit Banana Republic · M", not "Edit" — which is what a screen reader reads and what a paused pointer shows; it renders through `Glyph` and `EDIT_GLYPH`/`DELETE_GLYPH` are the shared verbs. `SizeRow` in `components/sizes.js` is the pattern in use: the name on the left, the value in a column down the right, and these actions at the end of the line, quiet until the row is hovered or reached by keyboard and always shown where there is no pointer. Reach for it instead of stacking an `ActionGroup` under every row, which doubles a list's length and makes its loudest thing the word Delete.
