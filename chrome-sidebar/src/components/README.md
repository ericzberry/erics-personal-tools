...
`CapabilityNavigation` renders the shared expandable sidebar navigation from `src/capabilities.js`, `CapabilityLauncher` the icon grid, and `CapabilityMenu` the mobile dropdown that wraps that grid. All three group entries by the registry's `section`, so an entry moves in both hosts at once; the sidebar shows a named section as one row that opens to reveal its tools, so give every section an icon in `src/capabilities.js`. Keep feature-specific actions inside their screens. The sidebar menu leads with a Home row (`HOME_CAPABILITY`, the same entry the mobile launcher's Home tile uses), which selects the home screen like any tool. The sidebar controller distinguishes Current tab automatic mode — which the toggle names and the menu does not list — from explicit capability selection, and marks the selected row with `aria-current` alone; Escape closes the list and returns focus to its summary. Mobile starts on Home with the grid shown in place and its summary hidden; once a tool is open the summary is the menu button, which wears the shared `Select` trigger's chrome — surface, border, and a caret that flips — and the open grid is drawn as the panel joined to its lower edge.

`ExpandableRecord` and `RecordRow` build ruled record lists, and `RecordGroup` in `ui.js` heads a run of them with a `GroupTitle` category label, ruled off from its records by `.record-group-title` — the shared form of the one-line group that Travel, Personal, Sizes and Reminders each still keep a private copy of (`TravelGroup` and its siblings); new groupings take the shared one. `ExpandableRecord` owns the disclosure marker: the whole heading line is the toggle, `travel.css` draws the marker as a stroked chevron in a fixed right-hand column on `.record-row-toggle::after`, and an open row takes the surface fill so its number, metadata, actions, and confirmation read as one block. What the row opens for goes on a `.record-number-line`: the value along the left and the record's own `RowAction` verbs at the end of it, passed as `preview` where the value is always shown and inside `children` where opening the row is what brings it out. Grouping order comes from `groupTravelRecords` in `travel-data.js`, which follows the category registry and keeps retired categories rather than dropping their records — group in the data helper, not in a feature controller. Do not append a marker character to a record's name or restyle these rows per feature; the rules live in [DESIGN.md](../../../docs/DESIGN.md#record-layout).

`RowAction` and `RowLink` are what a record's own actions look like: a 24x24 stroked glyph through `IconButton`, carrying the label that names the record it would act on — "Edit Banana Republic · M", not "Edit" — which is what a screen reader reads and what a paused pointer shows. The shared verbs are `EDIT_GLYPH`, `DELETE_GLYPH`, `DONE_GLYPH`, `UNDO_GLYPH`, `SHOW_GLYPH`, `HIDE_GLYPH`, `COPY_GLYPH`, `NOTES_GLYPH`, `OPEN_GLYPH` and `HISTORY_GLYPH`; `RowLink` is the same action where it opens a page elsewhere. `travel.css` owns how they wait at the end of `.record-line` and appear on hover, keyboard focus, or touch, and how a paused one — disabled while its tool saves — is dimmed to half wherever it shows (UI-56).
...
`FindField` is the find-record field above a record list, and the only search box: every tool builds its find field with it, and `select.css` draws it at the Filter density under a pointer and at the touch size under a finger by setting the field family's tokens on `.find-field`. A `Field` with no kind is a text box. The rule and its checks are UI-55 in [UI_RULES.md](../../../docs/UI_RULES.md).
