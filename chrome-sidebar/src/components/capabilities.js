...
export function CapabilitiesView(){
  return Stack([CapabilityPicker(),Section([Heading('Cloud connection',2),Stack([],{id:'capability-connection'})],{id:'capability-settings',className:'travel-wallet connection-only',hidden:true}),...CAPABILITIES.map(c=>Section([],{id:`capability-${c.id}`,hidden:true}))]);
}
