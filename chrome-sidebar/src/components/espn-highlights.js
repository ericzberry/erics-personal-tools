...
var EspnRecommendationHighlights = (() => {
  ...
  let desiredBackgrounds=new Map();
  function applyBackgrounds(){
    for(const [element,previous] of backgrounds){
      if(desiredBackgrounds.has(element))continue;
    ...
    for(const [element,color] of desiredBackgrounds){
      const priority=typeof element.style.getPropertyPriority==='function'?element.style.getPropertyPriority('background-color'):'';
      if(!backgrounds.has(element))backgrounds.set(element,{value:element.style.getPropertyValue('background-color'),priority});
      if(element.style.getPropertyValue('background-color')!==color||priority!=='important')element.style.setProperty('background-color',color,'important');
    }
  }
  ...
  function paintRow(row,color){
    ...
    for(const element of surfaces)desiredBackgrounds.set(element,color);
  }
  ...
  function decorate(document,candidates,tierPlayers=[],scarcityPlayers=[]){
    ...
    for(const row of rows){
      ...
      const nativeIds=[...row.querySelectorAll('[data-player-id]')].map(el=>Number(el.getAttribute('data-player-id'))).filter(Number.isFinite);
      const matches=p=>{
        if(nativeIds.length)return nativeIds.every(id=>id===Number(p.espnId));
      ...
      if(index<0)continue;
      recommended++;row.classList.add('eric-recommended-row');paintRow(row,'rgb(213, 230, 255)');
    }
    applyBackgrounds();
