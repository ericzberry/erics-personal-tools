(() => {
  ...
  function refresh(){
    ...
    EspnRecommendationHighlights.decorate(document,valid&&current?message.candidates:[]);
  }
  chrome.runtime.onMessage.addListener((next,sender)=>{
    if(sender.id!==chrome.runtime.id||next?.type!=='DRAFT_RECOMMENDATIONS')return;
    message=!next.clear&&Array.isArray(next.candidates)&&next.candidates.length<=2&&Number.isFinite(next.expiresAt)?next:null;
    refresh();
  });
