...
const changes=travelChanges(()=>tool.refresh({quiet:true}),{resource:'rewards'});
window.addEventListener('pagehide',()=>changes.close(),{once:true});
const tool=mountRewards(root,{credentials,offline,programs,remote:cloudRequest,onChanged:()=>changes.publish(),onSettings:()=>document.getElementById('open-settings')?document.getElementById('open-settings').click():location.assign('settings.html')});
...
document.getElementById('navigate-rewards')?.addEventListener('click',()=>tool.refresh({quiet:true}));
...
document.addEventListener('visibilitychange',()=>{if(!document.hidden&&!document.getElementById('rewards-tool')?.hidden)tool.refresh({quiet:true});});
