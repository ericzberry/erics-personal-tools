...
export function mountPageStrip(root,{
  ...
  let page={url:'',site:null},saved=[],stopped=false;
  function render(){
    const offers=pageOffers({...page,gifts:saved,active:active()});
    root.replaceChildren(...PageOfferStrip(offers,{onSelect:offer=>offer.viaTab?toTab():select(offer.capability)}));
  ...
  async function load(){
    ...
    // works.
    try{const token=await credentials.get();saved=token?await gifts.saved(token):[];}
    catch{saved=[];}
    if(!stopped)render();
  }
  const watch=changes(load,{resource:'gifts'});
  ...
  return {
    ...
    refresh:load,
    stop(){stopped=true;watch.close();unsubscribe();}
  };
}
