...
export function mountTravel(root,{credentials,request,offline,connectionRoot,onConnectionChange=()=>{},mode='inline',editId=null,onOpenEditor=()=>{},onSaved=()=>{},onDone=()=>{},clipboard=globalThis.navigator?.clipboard}) {
  ...
  function loadEditor(){
    ...
    edit(record);
    if(editorMissing)status('This record is no longer available. Return to the wallet and choose another record.','form-status');
  }
  $('add').addEventListener('click',()=>run(async()=>{await onOpenEditor();status('Editor opened in a new tab.');}));
  $('done').addEventListener('click',()=>{if(dirty){status('Save or cancel your edits before closing.','form-status');return;}onDone();});
  async function refresh(){const result=await request(token,'/v1/travel');records=result.records;if(!dirty)loadEditor();render();onConnectionChange();return result.syncMessage||'Travel records are up to date.';}
  $('connect').addEventListener('click',()=>run(async()=>{
    ...
    $('token').value='';$('cloud').open=false;edit();loadEditor();render();status(result.syncMessage||'Connected. Travel records are up to date.');onConnectionChange();
  }));
  ...
  $('form').addEventListener('submit',event=>{event.preventDefault();if(mode==='browse'||editorMissing)return;run(async()=>{
    ...
    edit();loadEditor();render();onSaved(savedId);status(result.syncMessage||'Saved. Available on your other devices when they refresh.');
  },'form-status');});
