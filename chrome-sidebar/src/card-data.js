export const PURCHASE_CATEGORIES=['Dining','Groceries','Gas','Travel','Transit','Online shopping','Drugstores','Entertainment','Other'];
...
export function normalizePurchase(input){
  ...
  const amount=number(input.amount,10000000,'purchase amount');if(amount<=0)fail('Enter a purchase amount greater than zero.');
...
export function compareCards(cards,input,today=new Date().toISOString().slice(0,10)){
  ...
  return cards.filter(card=>!card.deleting&&!card.conflict).map(raw=>{
    ...
    let units=purchase.amount*card.base/100,matched=null;
    for(const [index,rule] of rewardRules(card.rules).entries()){
      ...
      const eligible=rule.remaining===null?purchase.amount:Math.min(purchase.amount,rule.remaining);
      const earned=(eligible*rule.rate+(purchase.amount-eligible)*card.base)/100;
      if(earned>units){units=earned;matched=rule;}
    }
    const dollars=units*card.cpp;
    return {...card,dollars,rate:dollars/purchase.amount*100,earned:card.unit==='points'?units*100:units,matched,warnings};
