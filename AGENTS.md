...
| Hotel/flight research, comparisons and sign-in checkpoints | [travel planning](docs/TRAVEL_PLANNING.md) |
...
- Two different concepts on one screen are two tabs, not one long page. A tool that answers more than one question — what the ledger comes to, and how a figure gets into it — asks the questions across the top through the shared `Tabs` and answers one at a time. The tab's label is the heading for what is under it, so nothing inside repeats it; every label names a view rather than an action; a tab appears only while it has something to answer; and one visible tab draws no row. Do not overdo it: a row earns its place only where each tab is worth a screen of its own, so a short section, an editor drawer or an empty second view stays where it is, and most tools have no row at all. This is inside a tool: the shell's function still follows the browser tab, and no tool becomes a tab in the header. The full definition is in [docs/DESIGN.md](docs/DESIGN.md#tabs).
...
- Publish only after the build has passed its checks, been packaged, and been committed and pushed. Never publish an unfinished build or another agent's in-progress manifest version. Verify the version returned by `https://tools.ezberry.net/v1/releases/latest` matches the release being delivered.
...
- Give every capability an `icon` in the same change that registers it: a 24x24 stroked SVG path drawn with `currentColor`, rendered through the shared `Glyph` component. Both hosts show one icon per entry — the mobile launcher and the sidebar Tools menu — so add the icon to every list the capability appears in, including the sidebar-only entries. Keep the launcher alphabetical by label; registry order sets only the default tool.
