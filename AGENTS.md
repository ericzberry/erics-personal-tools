...
- Two different concepts on one screen are two tabs, not one long page. A tool that answers more than one question — what the ledger comes to, and how a figure gets into it — asks the questions across the top through the shared `Tabs` and answers one at a time. The tab's label is the heading for what is under it, so nothing inside repeats it; every label names a view rather than an action; a tab appears only while it has something to answer; and one visible tab draws no row. This is inside a tool: the shell's function still follows the browser tab, and no tool becomes a tab in the header. The full definition is in [docs/DESIGN.md](docs/DESIGN.md#tabs).
