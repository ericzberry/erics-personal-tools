...
import {searchInput,localDate,partySizes,isNYC,bookingURL,searchTimes} from './shared/restaurant-search.js';
...
export function mountRestaurants(root,{credentials,request,cache,loadConnections,online=()=>navigator.onLine!==false}) {
  ...
  const fields={mode:'mode',query:'query',city:'city',neighborhood:'neighborhood',date:'date',start:'startTime',end:'endTime',party:'partySize',min:'minParty',max:'maxParty',limit:'limit'};
  ...
  function visibility(){
    const category=$('mode').value==='category';
  ...
  function fill(value={}){
    const defaults={mode:'restaurant',query:'',city:'New York City',neighborhood:'',date:localDate(),startTime:'17:00',endTime:'22:00',partySize:value.minParty||2,minParty:2,maxParty:6,limit:12,...value};
    for(const [id,key] of Object.entries(fields))$(id).value=defaults[key];
    $('flexible').checked=!!value.flexible;$('travel').checked=!!value.includeLongTravel;
    $('date').min=localDate();
  ...
  function render(){
    ...
    if(!snapshot)return;
    const {search,research,downloadedAt}=snapshot;
    ...
    $('candidates').replaceChildren(...research.restaurants.map(r=>MobileRestaurantCandidate(r,{
      ...
      links:r.booking.flatMap(b=>partySizes(search).flatMap(size=>searchTimes(b.provider,search).map(time=>({provider:b.provider,size,time:['OpenTable','Tock'].includes(b.provider)?time:null,url:bookingURL(b.url,search,size,time)}))))
    })));
  }
  async function reloadConnections(){
    if(!online()){if(!connections.length)$('connection').replaceChildren(Option('Reconnect for research',''));controls();$('connection-status').textContent='Offline. Saved results are available; reconnect for new research.';return;}
    ...
    try{
      ...
      $('connection-status').textContent=connections.length?'Connected. Research uses live web sources.':'Add an OpenAI connection in the extension’s AI settings, then reload connections here.';
  ...
  async function find(event){
    ...
    try{
      ...
      search=searchInput({...input,flexible:$('flexible').checked,includeLongTravel:$('travel').checked});
  ...
  for(const id of ['mode','city','flexible'])$(id).addEventListener('input',visibility);
  ...
  return {
    async open(){
      ...
      try{snapshot=await cache.read();if(snapshot){fill(snapshot.search);render();$('search-panel').open=false;}}
