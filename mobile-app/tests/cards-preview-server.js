...
let rewards={entries:[
  ...
  // The cards the wallet knows the owner holds, as Best card sees them: one it
  // already has the rates for, one it does not, and one that only an issuer's
  // own benefits page ever named.
  {id:'77777777-7777-4777-8777-777777777777',kind:'card',name:'Synthetic Everyday Cash Card (-31002)',source:'Synthetic Bank',value:'2% back on everything',due:'',state:'available',url:'',notes:'',secret:'',secretHint:'1002',updatedAt:new Date().toISOString()},
  {id:'88888888-8888-4888-8888-888888888888',kind:'card',name:'Synthetic Reserve Card with a very long product name (-61007)',source:'Synthetic Bank',value:'3x travel and dining',due:'',state:'available',url:'',notes:'',secret:'',secretHint:'',updatedAt:new Date().toISOString()},
  {id:'aaaaaaaa-9999-4999-8999-999999999999',kind:'benefit',name:'Saks Fifth Avenue credit',source:'Synthetic Bank',value:'$50 twice a year',due:'',state:'available',url:'',notes:'',secret:'',secretHint:'',cadence:'semiannual',remaining:'$50',card:'77777777-7777-4777-8777-777777777777',updatedAt:new Date().toISOString()},
...
let cards=[{id:'22222222-2222-4222-8222-222222222222',name:'Synthetic Everyday Cash',unit:'cash',base:2,cpp:1,rules:JSON.stringify([{category:'Department stores',channel:'Any',rate:10,remaining:null,active:true,end:'',condition:'',merchant:'Saks Fifth Avenue'}]),checked:'2026-09-09',source:'https://example.com/terms',notes:'Synthetic terms for testing only.',revision:'first'}, {id:'33333333-3333-4333-8333-333333333333',name:'Synthetic Dining Points',unit:'points',base:1,cpp:1.5,rules:JSON.stringify([{category:'Dining',channel:'Any',rate:3,remaining:50,active:true,end:'',condition:'Eligible restaurant purchase'}]),checked:'2026-09-09',source:'',notes:'',revision:'first'}, {id:'44444444-4444-4444-8444-444444444444',name:'Synthetic Everyday Points',unit:'points',base:1,cpp:1.2,rules:JSON.stringify([{category:'Gas',channel:'Any',rate:4,remaining:null,active:true,end:'',condition:'',merchant:''},{category:'Department stores',channel:'Any',rate:3,remaining:null,active:true,end:'',condition:'',merchant:''}]),checked:'2026-09-09',source:'',notes:'',revision:'first'}];
...
createServer(async (req, res) => {
  try {
    ...
    if(url.pathname==='/cards-fixture.js'){
      res.setHeader('Content-Type','text/javascript');res.end(`import {mountCards} from '/app/shared/cards.js';import {cardsOffline} from '/app/shared/cards-offline.js';import {rewardsOffline} from '/app/shared/rewards-offline.js';import {CapabilityPicker} from '/app/shared/components/capabilities.js';const token='${token}';const remote=async(_t,path,options={})=>{const response=await fetch(path,{method:options.method||'GET',headers:{Authorization:'Bearer '+token,'Content-Type':'application/json'},body:options.value?JSON.stringify(options.value):undefined});const value=await response.json();if(!response.ok)throw Object.assign(Error(value.error||'Synthetic failure'),{status:response.status});return value;};document.getElementById('navigation').replaceChildren(CapabilityPicker());mountCards(document.getElementById('root'),{credentials:{get:async()=>token},offline:cardsOffline({remote}),wallet:rewardsOffline({remote}),remote});`);return;
    }
