...
Health (0.1.229) is the notebook of personal and family history, behind the same gate as Finance and Personal information and usable offline once downloaded; see [docs/HEALTH.md](../docs/HEALTH.md).

Unlocking the app also opens its protected sections. Finance, Personal information, Health, and the wallet's card numbers are sealed with a second key derived from the same passkey, so the unlock assertion evaluates both salts at once and hands the record key to the private frame. One verification covers the app and the records it holds for the same one-hour window; a provider that evaluates only one salt still unlocks the app, and each protected section then asks for itself. The record key is never persisted and is dropped with the token when the app locks. Because the app's lock governs that session, protected sections show no Lock now or recovery controls of their own on mobile; recovery runs through Recover access on the lock screen.
...
Choose **Restaurants** in Tools for the same search as the extension: one box for the whole request, said the way it would be said — *Sushi for 3 in the LES within 15 minutes of 12:15 this Saturday* — read into the day, the hour, the window and the party, with the city, those fields and the preferences behind **Details**. Results are one block per restaurant — the reason, the price basis, what is not verified, and **Details** with every claim, its source and whether it was read there. Each result shows **Check on Resy** (one per provider) with the date and party filled in, rebuilt as the date changes without new research. The iPhone web app cannot inspect other sites' signed-in tabs, so it never says a table is or is not there; automatic page inspection remains an extension capability. See [restaurants](../chrome-sidebar/RESTAURANTS.md).

The last ten searches — the request as understood, the candidates with their claims and sources, and the day's observations — are encrypted in device storage under the shared `restaurants` store and restored on a cold offline reopen with their dates. An earlier version's single download is carried forward as a dated search. New research needs internet; failed and cancelled research keep the last search; a storage failure is said and the results stay on screen. Disconnect clears the restaurant history with the other private copies. Browser storage remains evictable.
